...
def create_fat_binary(x86_bin_path, arm_bin_path, output_path):
    """Merge x86 and ARM64 kernel binaries into a single fat binary."""

    if not os.path.exists(x86_bin_path):
        print(f"[PACKER ERROR] Missing: {x86_bin_path}")
        sys.exit(1)
    if not os.path.exists(arm_bin_path):
        print(f"[PACKER ERROR] Missing: {arm_bin_path}")
        sys.exit(1)
    ...
    # Pack Main Fat Header: Magic, Count (Big-Endian for cross-arch safety)
    header = struct.pack('>II', FAT_MAGIC, arch_count)

    # Pack Architecture Descriptors
    ...
    total_size = len(header) + x86_size + arm_size
    print(f"[PACKER SUCCESS] Linked {output_path} ({total_size} bytes)")
    print(f"  x86_64 slice: {x86_size} bytes at offset {x86_offset}")
    print(f"  arm64  slice: {arm_size} bytes at offset {arm_offset}")
